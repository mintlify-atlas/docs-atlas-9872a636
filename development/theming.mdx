---
title: "Extend or customize leaftext themes with the token contract"
sidebarTitle: "Theming"
description: "leaftext enforces a compile-time semantic token contract of ~100 CSS custom properties. Learn how themes are defined, compiled, and validated in the source."
---
...
<Accordion title="Editor and Markdown elements">
...
<Accordion title="Alert callout colors">
...
<Accordion title="Syntax highlighting tokens">
...
<Accordion title="Minimap colors">
...
<Accordion title="Navigation chrome">
...
<Steps>
  <Step title="Define a new token map">
  ...
  <Step title="Register a ThemeSource">
  ...
  <Step title="Run the verification suite">
    ```sh
    just verify
    ```
  ...
  <Step title="Add the option to the Settings menu">
...
<Warning>
  The compile-time token check uses exact string matching against the names in
  `LEAF_SEMANTIC_TOKEN_CONTRACT`. Ensure every token name in your new theme map is
  spelled exactly as it appears in that list — a single typo (e.g.
  `--leaf-sytax-keyword` instead of `--leaf-syntax-keyword`) will not match and
  the assertion will fail at startup with a "missing required token" message.
</Warning>
